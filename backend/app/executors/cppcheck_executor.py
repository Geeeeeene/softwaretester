        ...
        try:
            ...
            for dir_path in target_directories:
                ...
                if path.exists() and path.is_dir():
                    ...
                    print(f"     [Debug] ✓ 目录存在: {path}")
                else:
                    print(f"     [Warning] ✗ 目录不存在: {path}")
            ...
            # 执行 Cppcheck
            # 使用UTF-8编码，如果失败则用replace替换无效字符
            result = subprocess.run(
                cmd,
                capture_output=True,
                text=False,  # 先获取字节，然后手动解码
                timeout=600,  # 10分钟超时
                cwd=cwd,
                env=env
            )
    ...
    def _parse_xml_output(self, xml_output: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for error in root.findall('.//error'):
                ...
                # Cppcheck XML可能有多个location节点，优先使用第一个有效的
                ...
                if locations:
                    ...
                    # 如果第一个location的行号无效，尝试其他location
                    if not line_str or line_str == "0":
                        for loc in locations[1:]:
                            loc_line = loc.get("line")
                            if loc_line and loc_line != "0":
                                line_str = loc_line
                                column_str = loc.get("column")
                                file_path = loc.get("file", file_path)
                                break
                else:
                    # 如果没有 location 节点，尝试从 error 属性获取
                    file_path = error.get("file", "unknown")
                    line_str = error.get("line")
                    column_str = error.get("column")
                
                # 调试日志
                if error_id not in filtered_ids:  # 只记录非过滤的错误
                    print(f"     [Debug] 解析错误: id={error_id}, file={Path(file_path).name if file_path != 'unknown' else 'unknown'}, line_str={line_str}, column_str={column_str}")
                ...
                # 解析行号和列号
                # 如果line_str是None、空字符串或"0"，表示文件级别的问题
                line_num = None
                if line_str is not None:
                    line_str_clean = str(line_str).strip()
                    if line_str_clean and line_str_clean != "0":
                        try:
                            line_num = int(line_str_clean)
                            if line_num <= 0:
                                line_num = None
                        except (ValueError, TypeError) as e:
                            print(f"     [Debug] 行号解析失败: line_str={line_str}, error={e}")
                            line_num = None
                
                column_num = None
                if column_str is not None:
                    column_str_clean = str(column_str).strip()
                    if column_str_clean and column_str_clean != "0":
                        try:
                            column_num = int(column_str_clean)
                            if column_num <= 0:
                                column_num = None
                ...
                # 调试：记录解析结果
                if error_id not in filtered_ids and line_num is None and line_str:
                    print(f"     [Warning] 行号解析为None但line_str存在: line_str={line_str}, file={Path(file_path).name}")
                ...
                # 如果行号为None，在消息中说明这是文件级别的问题
                final_message = message.strip() if message else "Unknown issue"
                if line_num is None and file_path != "unknown":
                    # 检查消息中是否已经包含行号信息
                    if "line" not in final_message.lower() and "at line" not in final_message.lower():
                        final_message = f"{final_message} (文件级别问题)"
                
                issue = {
                    "file": file_path,
                    "line": line_num,  # None表示文件级别问题，前端应该显示为"未知"或"文件级别"
                    "column": column_num,
                    "severity": severity,
                    "id": error_id,
